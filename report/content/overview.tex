...
Power BI là một nền tảng phân tích và trực quan hóa dữ liệu kinh doanh (Business Intelligence) do Microsoft phát triển. Công cụ này cho phép người dùng kết nối liền mạch với nhiều nguồn dữ liệu khác nhau, thực hiện làm sạch, mô hình hóa và xây dựng các báo cáo (reports) cũng như bảng điều khiển (dashboards) mang tính tương tác cao, giúp doanh nghiệp dễ dàng rút ra insight và ra quyết định dựa trên dữ liệu (data-driven).

\subsection{Các thành phần chính}
Hệ sinh thái Power BI bao gồm ba thành phần cốt lõi:
\begin{itemize}
    \item \textbf{Power BI Desktop:} Ứng dụng máy tính miễn phí trên Windows, đóng vai trò là công cụ chính để kết nối dữ liệu, viết DAX, mô hình hóa (Data Modeling) và thiết kế báo cáo.
...
\begin{itemize}
    ...
    \item \textbf{Data Transformation (Power Query):} Công cụ ETL (Extract, Transform, Load) tích hợp giúp dọn dẹp (clean), chuẩn hóa kiểu dữ liệu, gộp bảng (merge/append) một cách tự động thông qua giao diện trực quan hoặc ngôn ngữ M.
    \item \textbf{Data Modeling:} Xây dựng mô hình dữ liệu (như Star Schema) bằng cách thiết lập các mối quan hệ (Relationships) giữa các bảng (Many-to-One, One-to-One) và sử dụng ngôn ngữ DAX (Data Analysis Expressions) để tính toán các độ đo (Measures) phức tạp.
...
Đồ án sử dụng bộ dữ liệu \textbf{Retail Data Warehouse}, mô phỏng hoạt động của một doanh nghiệp bán lẻ đa chi nhánh quy mô lớn. Bộ dữ liệu chứa hơn 1 triệu dòng với 12 bảng quan hệ chặt chẽ với nhau, bao phủ toàn bộ vòng đời kinh doanh: từ chuỗi cung ứng (nhà cung cấp, sản phẩm), bán hàng (đơn hàng, khuyến mãi) cho đến hậu cần (vận chuyển, thanh toán, hoàn trả). 
...
Để giải quyết toàn diện bài toán, nhóm đã chia nhỏ Dashboard thành 4 luồng phân tích chuyên sâu, tương ứng với 4 thành viên:

\begin{enumerate}
    \item \textbf{Dương Tuấn Anh - Phân tích Xu hướng Doanh thu (Sales Trend):} 
    ...
    \item \textbf{Lê Xuân Trí - Phân tích Khách hàng (Customers):}
    ...
    \item \textbf{Nguyễn Trần Trung Kiên - Phân tích Sản phẩm (Products):}
    ...
    \item \textbf{Lê Đức Phúc - Phân tích Vận hành \& Tổng quan (Operations \& Overview):}
    Đánh giá năng lực của hệ thống chi nhánh và rủi ro logistics. Phân tích chi tiết tình trạng giao hàng (Delivered, Shipped, Late) kết hợp với năng suất nhân sự. Cuối cùng, tổng hợp các KPI của cả 3 nhánh trên vào một màn hình Overview dành cho cấp C-Level.
\end{enumerate}